// Log EVERY velocity kink of the Left Arch trajectory with the zone open.
...
for (let i = 0; i < 500; i++) {
    ...
    const gDrop = new THREE.Vector3(0, -CS2.gravity * 0.4 * CS2.TICK, 0);
    const expect = vin.clone().add(gDrop);
    if (expect.angleTo(v) > 0.05 || Math.abs(v.length() - expect.length()) > 20) {
        console.log(`t=${(i/64).toFixed(2)}s @ game(${pos.z.toFixed(0)}, ${pos.x.toFixed(0)}, ${pos.y.toFixed(0)})  h ${Math.hypot(vin.x, vin.z).toFixed(0)}->${Math.hypot(v.x, v.z).toFixed(0)}  vy ${vin.y.toFixed(0)}->${v.y.toFixed(0)}`);
    }
}
console.log(`REST game (${pos.z.toFixed(0)}, ${pos.x.toFixed(0)}, ${pos.y.toFixed(0)})`);
