// Trace the tail of the Left Arch trajectory: what speed does the nade carry
// into the (-1203,360) touchdown, and why does it stop there?
...
while (grenades.stepProjectile(nade, CS2.TICK, false) && t < 768) {
    ...
    // log every bounce (vy flips up) and the last phase
    if ((prevY < -50 && v.y > 0) || t / 64 > 2.8) {
        if ((prevY < -50 && v.y > 0) || t % 16 === 0) {
            console.log(`t=${(t/64).toFixed(2)}s  game (${pos.z.toFixed(0)}, ${pos.x.toFixed(0)}, ${pos.y.toFixed(0)})  h=${h.toFixed(0)} vy=${v.y.toFixed(0)} ${nade.rolling ? 'ROLL' : ''}`);
        }
    }
    prevY = v.y;
}
console.log(`REST t=${(t/64).toFixed(2)}s  game (${pos.z.toFixed(0)}, ${pos.x.toFixed(0)}, ${pos.y.toFixed(0)})`);
